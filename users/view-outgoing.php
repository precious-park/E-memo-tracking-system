...
<body class="bg-gray-100 flex flex-col min-h-screen">
  ...
  <script>
        ...
        "columns": [{
          ...
          },
          {
            ...
            "render": function(data, type, row) {
              // Pass the row as second argument
              return `<button class="bg-green-500 hover:bg-amber-600 text-white px-3 py-1 rounded" onclick="sendMemo(${row.memo_id}, this.closest('tr'))">Send</button>`;
            }
          }
        ]
      });
    ...
    function sendMemo(memoId, row) {
      Swal.fire({
        title: 'Send Memo?',
        text: "Are you sure you want to send this memo?",
        icon: 'warning',
          ...
          $.ajax({
            ...
            success: function(response) {
              if (response.status === 'success') {
                ...
                const table = $('#memosTable').DataTable();
            ...
            error: function(xhr, status, error) {
              Swal.fire('Error', 'An error occurred while sending the memo.', 'error');
              console.error(error);
            }
          });
        }
